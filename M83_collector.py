...
WIU_Title = 'M83_Collector 1.0 - by M83 visual effect Studio'
WIU_Log = '[M83_Collector] '
...
class Ui_Dialog(object):
	def setupUi(self, Dialog):
		...
		self.CopyFontDir.setChecked(False)
		...
		self.CopyGizmos.setChecked(False)
	...
	def retranslateUi(self, Dialog):
		...
		Dialog.setWindowTitle(_translate("Dialog", "M83 Collector"))
		...
		self.LWebsite.setToolTip(_translate("Dialog", "M83 Collector Menual"))
		self.LWebsite.setText(_translate("Dialog", "<html><head/><body><p><a href=\"https://m83.atlassian.net/wiki/spaces/MANUAL/pages/879067137/M83+Collector\"><span style=\" text-decoration: underline; color:#6bd5ff;\">M83 Collector Menual</span></a></p></body></html>"))
...
def _Start(silent = False, nk = '', startnow = False, out = '', nodenamefolder = True, parentdircount = 3, relinked = True, relativerelinked = True, media = True, fonts = False, gizmos = False, csvcommas = False, licinteractive = False):
	...
	def PackedPath(fullPath, i = 0, nodeName = '', fontFolder = '', project_dir = False, settingFontPath = False):
		...
		if i == 0:
			...
			if 'MetadataCopy_' in newPath:
				newPath = newPath.replace('MetadataCopy_', '')
	...
	def ProcessFile(fileFrom, fileTo, writeInit, firstInit, relinkMethod, relinkFonts):
		...
		try:
			...
			if relinkMethod is not -1:
				...
				# temp terminal file
				filePath = WIU_PackedPath + '/M83_collector_Temp-RELINK_' + str(relinkMethod) + '.py'
	...
	# apply read data to M83_collector window
	def RefreshUI():
		...
		for dataItem in WIU_MediaData:
			...
			if 'MetadataCopy ' in nodeName:
				nodeName = nodeName.replace('MetadataCopy ', '')
			...
			# data size check
			DataSize = BytesToString(dataItem[5]).split(' ')
			...
			if not silent:
				...
				if(existsBool):
					...
					# add item / add data to item / count
					if "/storage/m83_lib/" in item:
						WListIgnorePaths.addItem(item)
						WListIgnorePaths.item(iIgnore).setData(QtCore.Qt.UserRole, i)
						if DataSize[-1] == 'GB' and float(DataSize[0]) >= 2.5:
							WListIgnorePaths.item(iIgnore).setForeground(QtGui.QColor(255, 255, 200))
						iIgnore += 1
					elif "/core/" in item:
						WListIgnorePaths.addItem(item)
						WListIgnorePaths.item(iIgnore).setData(QtCore.Qt.UserRole, i)
						...
						if DataSize[-1] == 'GB' and float(DataSize[0]) >= 2.5:
							WListCopyPaths.item(iCopy).setForeground(QtGui.QColor(255, 255, 200))
						iCopy += 1
	...
	def UpdateLabels(CopyList):
		...
		if len(sel) == 1:
			...
			# get shortened path
			packeditemPreviewpath = packeditempath.replace(WIU_PackedPath, '[collection folder]')
			packeditempathShortened = shortenPath(packeditemPreviewpath, labelLength)
			# set as text
	...
	def ChoosePackedPath():
		...
		chosenPath = nuke.getFilename('M83 Collector - choose the folder to copy the script and all media to')
	...
	def ThreadedCopy(fileList, relinkFonts):
		...
		for f in fileList:
			...
			# interrupt check
			if WIU_Interrupted:
				WriteLog( CSV([timestr, 'PROCESS WAS CANCELED BY USER']) )
			...
			if result == '':
				...
				WriteLog( CSV([timestr, 'SUCCESS', f[0], f[1], getNodeNames(f[5], i=f[7]), getKnobNames(f[6]), BytesToString(f[3])]) )
	...
	def StopCopy():
		...
		if WIU_Copying:
			...
			if q:
				...
				WCurrentCopyItem.setText("<font color='red'>Canceling...</font>")
...
def M83_collector(fromterminal = False, nk = '', startnow = False, out = '', nodenamefolder = True, parentdircount = 3, relinked = True, relativerelinked = True, media = True, fonts = False, gizmos = False, csvcommas = False, licinteractive = False):

	global WIU_AppPath
	global dest
	global WIU_PackedPath

	os.environ['NUKE_JOB_VERSION'] = os.path.basename(nuke.root().name()).split('.')[0]
	if os.environ.get('SHOW'):
		dest = "/show/{SHOW}/seq/{SEQ}/{SEQ}_{SHOT}/comp/pub/{NUKE_JOB_VERSION}_backup".format(**os.environ)
	else:
		dest = ''
...
if __name__ == "__main__":
	...
	if t or ti:
		...
		if err:
			...
			M83_collector(fromterminal = True, nk = aNK, startnow = aStart, out = aOut, nodenamefolder = aNodeName, parentdircount = aDirCount, relinked = aReli, relativerelinked = aReliRela, media = aMedia, fonts = aFonts, gizmos = aGizmos, csvcommas = aCSV, licinteractive = aLicInteractive)
